...
import { ArrowLeft, Network, Sparkles, BrainCircuit, CheckCircle2, Circle, Lock, Filter } from 'lucide-react';
import {
  ReactFlow,
  MiniMap,
  Controls,
...
export function KnowledgeGraphView({ onClose }: KnowledgeGraphViewProps) {
  ...
  const baseNodes: Node[] = ctxNodes.map(cn => {
    ...
    return {
      id: cn.id,
      type: cn.type,
      data: { ...cn, label: cn.label, status: cn.status },
      position: pos
    };
  });
